...
def labRuntimeConfig : Html := Html.text false <| String.intercalate "\n" <| [
  ...
  "  residentHtmlExportName: \"VirPanelRegistry.formatHtmlById\"",
  "};",
  ...
  "};",
  "window.__versoPrettyLlvmHtmlConfig = {",
  "  adapterUrl: new URL(\"vir-pretty/lean-llvm-html/prettyM-html-emscripten-adapter.mjs\", window.location.href).href,",
  "  manifestUrl: new URL(\"vir-pretty/lean-llvm-html/prettyM-html.manifest.json\", window.location.href).href",
  "};"
] ++ sharedRevealPolicyConfig ++ ["</script>"]
...
def config : Config := {
  ...
  panelPlugins := #[
    ...
    "vir-pretty/pretty-llvm-html.js",
