...
public def formatHtmlByIdForVir (id width indent : Nat) :
    VersoSlides.Pretty.ResidentHtml :=
  VersoSlides.Pretty.formatHtmlAt formats annotationTables id width indent

end VersoSlides.PrettyRegistry
"""

    def panel_module(self, include_formatter_exports: bool = True) -> str:
        values = ",\n    ".join(format_expr(value) for value in self.formats)
        annotation_values = ",\n    ".join(
            annotation_table_expr(value) for value in self.annotations
        )
...
@[vir_export]
def formatHtmlById (id width indent : Nat) : Pretty.ResidentHtml :=
  Pretty.formatHtmlAt formats annotationTables id width indent
"""
        return f"""/- Generated by scripts/generate-pretty-registry.py. Do not edit. -/
...
/-- A second Verso component sharing this package and its bounded VIR runtime. -/
@[vir_export]
def planRevealPolicy
    (policy : VersoSlides.RevealPolicy.Policy)
    (event : VersoSlides.RevealPolicy.Event) :
    Array VersoSlides.RevealPolicy.Command :=
  policy.plan event
...
def main() -> None:
    ...
    parser.add_argument(
        "--panel-only",
        action="store_true",
        help="write only the resident panel mount/unmount surface to lean_output",
    )
    ...
    panel_modes = sum(
        [args.combined_panel, args.panel_only, args.panel_output is not None]
    )
    if panel_modes > 1:
        raise SystemExit(
            "--combined-panel, --panel-only, and --panel-output are mutually exclusive"
        )

    args.lean_output.parent.mkdir(parents=True, exist_ok=True)
    args.lean_output.write_text(
        registry.panel_module(include_formatter_exports=args.combined_panel)
        if args.combined_panel or args.panel_only
        ...
        metadata.update(
            {
                ...
                "htmlEntrypoint": (
                    "VirPanelRegistry.formatHtmlById"
                    if args.combined_panel
                    else "VersoSlides.PrettyRegistry.formatHtmlByIdForVir"
                ),
                "htmlOutput": "verso-token-html/v1",
            }
        )
    if args.combined_panel or args.panel_only or args.panel_output is not None:
        metadata.update(
            {
                "panelContentCount": len(registry.contents),
                "panelEntrypoint": "VirPanelRegistry.mountContent",
                "sharedComponentEntrypoints": ["VirPanelRegistry.planRevealPolicy"],
            }
        )
