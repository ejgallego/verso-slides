...
async def main() -> None:
    ...
    async with async_playwright() as playwright:
        ...
        assert await page.evaluate("getPrettyBackend('llvm-html') !== null")
        html_ids.append("llvm-html")
        ...
        assert await matrix.locator(".pretty-matrix-cell.is-unsupported").count() == (
            2 if native_html_available else 3
        )
        ...
        ).count() == (0 if native_html_available else 1)
        ...
        llvm_html_cell = matrix.locator(
            '.pretty-matrix-cell[data-pretty-backend="llvm"]'
            '[data-pretty-breadth="html"]'
        )
        assert await llvm_html_cell.locator(".pretty-matrix-choice").evaluate_all(
            "els => els.map(el => el.dataset.prettyCandidate)"
        ) == ["llvm-html"]
        assert "LLVM Wasm HTML" in await llvm_html_cell.inner_text()
        ...
        assert "vir-html-residency" in options
        ...
        llvm_html = await block.locator(
            '[data-pretty-backend="llvm-html"] .pretty-compare-body'
        ).inner_html()
        assert llvm_html == js_html, (
            "LLVM complete HTML differs from JavaScript complete HTML\n"
            f"JavaScript: {js_html!r}\n"
            f"LLVM: {llvm_html!r}\n"
            f"Console: {console_warnings!r}"
        )

        await experiment.select_option("vir-html-residency")
        await page.wait_for_function(
            "el => [...el.querySelectorAll('.pretty-compare-pane')].map(x => x.dataset.prettyBackend).join(',') === 'vir-html,vir-resident-html'",
            arg=await block.element_handle(),
        )
        assert await panes.evaluate_all(
            "els => els.map(el => el.dataset.prettyBackend)"
        ) == ["vir-html", "vir-resident-html"]
        resident_html = await block.locator(
            '[data-pretty-backend="vir-resident-html"] .pretty-compare-body'
        ).inner_html()
        assert resident_html == js_html, (
            "VIR resident HTML differs from JavaScript complete HTML\n"
            f"JavaScript: {js_html!r}\n"
            f"Resident VIR: {resident_html!r}\n"
            f"Console: {console_warnings!r}"
        )
        ...
        assert resident_boundary["interfaceExports"] == 11
