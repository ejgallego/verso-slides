...
test("rotates candidates and excludes warm-up samples", async () => {
    ...
    assert.deepEqual(order, ["left", "right", "left", "right", "right", "left", "left", "right"]);
...
test("a throwing or unavailable candidate cannot pass", async () => {
    ...
    assert.deepEqual(throwing.scenarios[0].candidates.broken.errors, [
        "RangeError: synthetic overflow",
    ]);

    const unavailable = await runDifferentialSamples({
        candidates: [{ id: "ready" }, { id: "missing", status: () => "unavailable" }],
        scenarios: [null],
        warmup: 0,
        samples: 1,
        yieldNow: async () => {},
