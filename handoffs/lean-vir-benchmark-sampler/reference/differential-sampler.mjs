...
export function summarizePhases(samples) {
    ...
    return Object.fromEntries(
        names.map((name) => [name, summarizeValues(samples.map((sample) => sample[name]))]),
    );
}
...
export async function runDifferentialSamples(options) {
    ...
    for (let scenarioIndex = 0; scenarioIndex < options.scenarios.length; scenarioIndex += 1) {
            ...
            try {
                ...
                if (!observation.ok) {
                    return {
                        ok: false,
                        error:
                            observation.error ?? "candidate returned an unsuccessful observation",
        ...
        function observe(result, observation) {
            if (!observation.ok) {
                result.stable = false;
                if (!result.errors.includes(observation.error))
