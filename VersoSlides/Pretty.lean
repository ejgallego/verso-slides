...
private def innermostAnnotationSlot
  ...
  if sorted && 1 < tags.size && !annotations.isEmpty then
    let last := annotations.size - 1
    if annotations[last]!.tag == tags[tags.size - 1]! then last + 1 else visit tags.size
  else
    visit tags.size
...
private def innermostAnnotation (annotations : Array TaggedAnnotation)
    (sorted : Bool) (tags : Array Nat) : Option TokenAnnotation :=
  let slot := innermostAnnotationSlot annotations sorted tags
    ...
    if s.isEmpty then
      ...
      modify fun st =>
        { st with
          nodes := st.nodes.push {
            text := s
            annotationSlot := innermostAnnotationSlot
              st.annotations st.annotationsSorted st.tagStack
          }
          column := st.column + String.Internal.length s }
  pushNewline indent :=
    modify fun st =>
      ...
      fun st =>
        ((), { st with
          chunks := st.chunks.push <| annotatedTextToHtml s <|
            innermostAnnotation st.annotations st.annotationsSorted st.tagStack
          column := st.column + String.Internal.length s })
  pushNewline indent :=
    fun st =>
      ((), { st with
...
The sparse annotation array need not use dense or ordered tag IDs. Ascending
tag order selects a binary-search fast path; unsorted input retains the same
lookup semantics. The result is independent of HTML and the DOM, but unlike
`Rendered` it needs no tag-stack reconstruction or annotation lookup in
JavaScript.
-/
public def formatRenderPlan (f : Std.Format)
  ...
  let nodes := (act.run {
    column, annotations, annotationsSorted := annotationTableSorted annotations
  }).2.nodes
