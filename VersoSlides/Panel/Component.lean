...
payloads, measuring the available width, and committing the view to the DOM.
...
public structure Model where
  ...
  /-- Available expression width in monospace columns. -/
  width : Nat := 40
deriving Inhabited
...
public def update (model : Model) : Event → Transition
  ...
  | .provideContent sourceId content =>
    if model.activeSource == some sourceId then
      { model := { model with content }, effects := #[.requestWidth] }
    else
      { model }
  | .resized width =>
    { model := { model with width := max 1 width } }
  | .clear =>
    { model := { model with activeSource := none, content := .empty }
