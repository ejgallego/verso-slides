...
def code_url(site_dir, server):
    ...
    assets = {
        ...
        "pretty-llvm-html.js": "pretty-llvm-html.js",
...
class TestPrettyVirAssets:
    def test_pretty_vir_asset_owned_by_demo(self):
        ...
        body = path.read_text()
        loader = (DEMO_WEB / "vir-loader.js").read_text()
        assert "createVirRuntime" in loader
        assert "irPackageSetBytes" in loader
        assert "./lean-vir/js/vir-runtime.js" in loader
        assert "./lean-vir/wasm/vir-upstream.wasm" in loader
        ...
        assert "VersoSlides.PrettyRegistry.formatHtmlByIdForVir" in body
    ...
    def test_pretty_llvm_html_asset_owned_by_demo(self):
        """The complete-HTML LLVM bootstrap is owned by the formatter demo."""
        path = DEMO_WEB / "pretty-llvm-html.js"
        assert path.exists(), f"Expected pretty-llvm-html.js at {path}"
        body = path.read_text()
        assert 'id: "llvm-html"' in body
        assert "loadEmscriptenPrettyMHtmlAdapter" in body
        assert "prettyM-html.manifest.json" in body
        assert "verso-token-html/v1" in body
...
class TestPrettyVirBridge:
    ...
    def test_format_to_html_can_force_backends(self, code_url: str, page: Page):
        """The prototype comparison path can render JS and active VIR outputs separately."""
        goto_slide_by_title(page, code_url, "Dark Code")
...
class TestPrettyLlvmHtmlBridge:
    def test_configured_html_adapter_registers_llvm_matrix_cell(
        self, code_url: str, page: Page
    ):
        """The LLVM HTML package fills the LLVM × HTML matrix cell."""
        goto_slide_by_title(page, code_url, "Dark Code")
        page.evaluate(
            """() => {
                ...
                window.__versoPrettyLlvmHtmlConfig = { enabled: false };
            }"""
        )
        script = (DEMO_WEB / "pretty-llvm-html.js").read_text()
        page.add_script_tag(content=script)
        result = page.evaluate(
            """() => ({
                candidate: getPrettyMatrixBackend('llvm', 'html'),
                allBackends: window.__versoPrettyConfig.experiments[0].backends,
                status: window.__versoPrettyLlvmHtml.status
            })"""
        )
        assert result["candidate"]["id"] == "llvm-html"
        assert result["candidate"]["capabilities"]["matrix"] == {
            "backend": "llvm",
            "breadth": "html",
        }
        assert result["allBackends"] == ["js", "llvm", "llvm-html"]
        assert result["status"] == "disabled"
...
class TestPrettyNativeFlatBridge:
    ...
    def test_configured_flat_adapter_registers_and_renders_utf8_events(
        ...
        assert result["experiment"]["design"] == "exploratory"
...
class TestPrettyVirComparisonPanel:
    ...
    def test_tactic_panel_renders_registered_backends_side_by_side(
        ...
        pane_ids = panel.locator(".pretty-compare-pane").evaluate_all(
            "panes => panes.map(pane => pane.dataset.prettyBackend)"
        )
        assert len(pane_ids) == 14
        assert "vir-resident-html" in pane_ids
    ...
    def test_controls_select_processors_and_share_column_budget(
        ...
        controlled_url = (
            code_url
            + "/index.html?prettyControls=1&prettyCompare=1"
            + "&pretty=js,vir-format&prettyColumns=32#/"
            + str(index)
        )
        page.goto(controlled_url)
        ...
        expect(
            controls.locator('.pretty-controls-backend input[value="vir-flat"]')
        ).not_to_be_checked()
        ...
        controls.locator('.pretty-controls-backend input[value="vir-format"]').uncheck()
