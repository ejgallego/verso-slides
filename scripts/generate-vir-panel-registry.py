#!/usr/bin/env python3
"""Generate a deck-resident VIR panel module and attach content IDs."""
...
def format_expr(value: Any) -> str:
    ...
    if tag == 3 and len(value) == 3 and isinstance(value[1], int):
...
class Registry:
    ...
    def panel_module(self) -> str:
        ...
        return f"""/- Generated by scripts/generate-vir-panel-registry.py. Do not edit. -/
import VirPanel
...
def mountContent (selector : String) (contentId : Nat)
  ...
  | some content =>
    ...
    Lean.Vir.React.Root.renderComponentIntoSelector selector
      VersoSlides.VirPanel.view {{ content, width := max 1 width, widths, measureOnly }}

@[vir_export]
def unmount (selector : String) : DomM Bool :=
  VersoSlides.VirPanel.unmount selector
...
def main() -> None:
    ...
    args.lean_output.write_text(registry.panel_module())
    metadata = {
        "schemaVersion": 1,
        "formatCount": len(registry.formats),
        "panelContentCount": len(registry.contents),
        "idField": "data-vir-panel-content",
        "mountEntrypoint": "VirPanelRegistry.mountContent",
        "unmountEntrypoint": "VirPanelRegistry.unmount",
    }
    args.metadata_output.parent.mkdir(parents=True, exist_ok=True)
    args.metadata_output.write_text(json.dumps(metadata, indent=2) + "\n")
    print(
        f"generated resident VIR panel with {len(registry.formats)} unique formats "
        f"and {len(registry.contents)} contents"
    )
